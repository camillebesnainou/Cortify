...
class _PlaylistScreenState extends State<PlaylistScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final List<String> priorityFiles = mediaFiles
        .where((file) =>
            playlists[selectedStimulusType]![file]['priority'] == true)
        .toList();
    final List<String> nonPriorityFiles = mediaFiles
        .where((file) =>
            playlists[selectedStimulusType]![file]['priority'] != true)
        .toList();
    final List<String> sortedMediaFiles = [
      ...priorityFiles,
      ...nonPriorityFiles
    ];
    final albums = sortedMediaFiles
        .map((file) => playlists[selectedStimulusType]![file]['album'])
        .toSet()
        .toList();
    final artists = sortedMediaFiles
    ...
    if (selectedStimulusType == "Audiobooks" ||
      ...
      if (selection.isNotEmpty) {
        ...
        sortedMediaFiles.removeWhere((file) =>
    ...
    if ((selectedStimulusType == "Audiobooks" ||
                ...
                child: Container(
                  ...
                  child: ListView.builder(
                    ...
                    itemBuilder: (ctx, index) {
                      ...
                      final numChapters = sortedMediaFiles
                          .where((file) =>
                              playlists[selectedStimulusType]![file][groupby] ==
                              artistOrAlbumSelected)
                          .length;
                      final representativeMediaFile =
                          sortedMediaFiles.firstWhere(
                        (file) =>
                            playlists[selectedStimulusType]![file][groupby] ==
                            artistOrAlbumSelected,
                      );
                      // Vérifie si la propriété 'priority' est vraie dans les métadonnées
                      Color tileColor;
      ...
      print('Group: $group');
      print('Media Files: $sortedMediaFiles');
                ...
                child: Stack(
                  children: [
                    ...
                    Positioned(
                      ...
                      left: 0.0,
                      right: 0.0,
                      child: (["Audiobooks", "Musique", "Podcasts"])
                    ...
                    Positioned(
                      top: (["Audiobooks", "Musique", "Podcasts"])
                              .contains(selectedStimulusType)
                          ? 60.0
                      ...
                      child: Container(
                        ...
                        child: ListView.builder(
                          itemCount: sortedMediaFiles.length,
                          itemBuilder: (ctx, index) {
                            // Builder function to create each item in the list
                            final mediaFile = sortedMediaFiles[
  ...
  Future<List<Widget>> _buildAudioListItem(
    ...
    // AUDIO
    if (["Audiobooks", "Musique", "Podcasts"].contains(stimulusType)) {
      if (stimulusType == "Audiobooks" || stimulusType == "Podcasts") {
        primaryLabel = '$title';
